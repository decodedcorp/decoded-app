...
- **목적**: 이미지와 검출된 아이템을 상세 표시, 핀 인터랙션 및 구매 연결 제공
- **선행 조건**: 유효한 이미지 ID
- **후속 화면**: 구매 링크 (외부), Cast/Media 페이지
- **관련 기능 ID**: [V-01](../../feature-spec/03-detail-interaction.md#v-01-responsive-detail-view)
...
| **BTN-X** | 버튼 | 닫기 | - Icon: × (24px)<br>- Position: Top-right | **Click**: router.back() |
| **IMAGE** | 이미지 | 메인 이미지 | - Priority: true<br>- Aspect: original | onLoad → FLIP 애니메이션 |
| **PIN-OVERLAY** | 오버레이 | 핀 컨테이너 | - Position: absolute<br>- Inset: 0 | 핀 마커 렌더링 |
| **CONNECTOR** | SVG | 연결선 | - Stroke: gray/primary<br>- Dasharray: 선택 시 solid | 핀-카드 시각적 연결 |
...
| **ITEM-CARD** | 카드 | 아이템 카드 | - Number: 1, 2, 3...<br>- Highlight: 선택 시 ring | **Click**: 해당 핀 선택<br>상세 정보 표시 |
...
| **핀 선택** | 핀 또는 카드 클릭 | 해당 핀/카드 하이라이트 + 연결선 활성화 |
...
│       │                                                  │               │
│       ├─── 핀: selected 스타일 (scale 1.2, primary)      │               │
...
| Local State | `selectedPinId` | `string \| null` | 현재 선택된 핀 |
...
| 이미지+핀 | InteractiveShowcase | `packages/web/lib/components/detail/InteractiveShowcase.tsx` |
| 핀 오버레이 | PinOverlay | `packages/web/lib/components/detail/PinOverlay.tsx` |
| 핀 마커 | PinMarker | `packages/web/lib/components/detail/PinMarker.tsx` |
