...
export function StepIndicator({ currentStep, totalSteps, selectedImage }: StepIndicatorProps) {
  ...
  // Display step 0 if no image is selected
  const displayedStep = selectedImage ? currentStep : 0;
    ...
    <div className="w-full mb-4">
      <div className="relative pt-1">
            ...
            {[...Array(totalSteps)].map((_, index) => (
              <div key={index} className="relative">
                ...
                <div
                  ...
                  `}
                >
                  {selectedImage && index + 1 <= currentStep && (
        ...
        <h2 className="mt-3 text-lg md:text-xl font-bold text-gray-400 text-center">
          {STEP_MESSAGES[displayedStep as keyof typeof STEP_MESSAGES]}
        </h2>
      </div>
    </div>
  );
