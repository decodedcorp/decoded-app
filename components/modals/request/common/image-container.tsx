...
function ensureImageWithinCropBox(cropper: any): boolean {
  ...
  if (canvasData.width < cropBoxData.width) {
    ...
    // 우측 경계 체크
    if (canvasData.left + canvasData.width < cropBoxData.left + cropBoxData.width) {
      adjustedCanvasData.left = cropBoxData.left + cropBoxData.width - canvasData.width;
      needAdjust = true;
    }
  }
  ...
  if (canvasData.height < cropBoxData.height) {
    ...
    // 하단 경계 체크
    if (canvasData.top + canvasData.height < cropBoxData.top + cropBoxData.height) {
      adjustedCanvasData.top = cropBoxData.top + cropBoxData.height - canvasData.height;
      needAdjust = true;
    }
  }
  ...
  (
    ...
    const ensureImageWithinCropBox = useCallback((cropper: any) => {
      try {
        ...
        // 오른쪽 경계 확인
        if (canvasData.left + canvasData.width < cropBoxData.left + cropBoxData.width) {
          newCanvasData.left = cropBoxData.left + cropBoxData.width - canvasData.width;
          needsUpdate = true;
        }
        ...
        // 아래쪽 경계 확인
        if (canvasData.top + canvasData.height < cropBoxData.top + cropBoxData.height) {
          newCanvasData.top = cropBoxData.top + cropBoxData.height - canvasData.height;
          needsUpdate = true;
        }
          ...
          if (cropperRef.current?.cropper) {
            ...
            if (canvasData.width > cropBoxData.width && canvasData.height > cropBoxData.height) {
              cropper.setCanvasData({
                left: cropBoxData.left + (cropBoxData.width - canvasData.width) / 2,
                top: cropBoxData.top + (cropBoxData.height - canvasData.height) / 2
              });
            }
          }
        }, 50);
        ...
        try {
          ...
          const centerImageAndSave = () => {
            try {
              ...
              cropperRef.current.initialCanvasData = cropperRef.current.cropper.getCanvasData();
              cropperRef.current.initialScale = cropperRef.current.cropper.getData().scaleX || 1;
            } catch (err) {
        ...
        if (originalImageState?.dataUrl && originalImageState?.file) {
          ...
          setTimeout(() => {
            ...
            try {
              ...
              setTimeout(() => {
                if (cropperRef.current?.cropper) {
                  cropperRef.current.cropper.replace(originalImageState.dataUrl);
        ...
        if (imageFile) {
          ...
          if (storedData && storedData.dataUrl) {
            ...
            fetch(storedData.dataUrl)
              .then(res => res.blob())
              .then(blob => {
                const originalFile = new File([blob], storedData.fileName || 'original.jpg', {
    ...
    }, [onImageSelect, originalImageState, handleCropperReady, imageFile, loadFromSessionStorage, onCropperChange]);
      ...
      try {
        ...
        if (
          ...
          requestAnimationFrame(() => {
            if (cropperRef.current?.cropper && cropperRef.current.initialCanvasData) {
              cropperRef.current.cropper.setCanvasData(cropperRef.current.initialCanvasData);
            }
          });
        } else {
          // 기본 리셋
    ...
    const applyCrop = useCallback((): Promise<void> => {
      return new Promise<void>((resolve, reject) => {
        ...
        try {
          ...
          const quality = Math.min(0.92, 800000 / (targetWidth * targetHeight * 0.3));
    ...
    const ImageViewer = useCallback(() => (
      <div className="relative w-full h-full overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute top-4 right-4 z-10">
          <button
            className="px-3 py-1.5 bg-[#333]/80 text-white rounded-md text-sm hover:bg-[#444]/80 transition-colors"
    ...
    }, [imageFile, getOriginalImageKey, onImageSelect, showCropper, handleCropperReady, setOriginalImageState, fallbackReset]);
        ...
        <div className="flex-1 relative flex items-center justify-center bg-[#1A1A1A] overflow-hidden">
          ...
          <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
            <button
              ...
              className="p-2 bg-[#333]/80 text-white rounded-md hover:bg-[#444]/80 transition-colors"
              title="확대"
    ...
    const MarkerComponent = useCallback(() => {
      console.log("Rendering MarkerComponent with image:", selectedImage ? "provided" : "missing");
    ...
    const ReadOnlyMarkerComponent = useCallback(() => {
      console.log("Rendering ReadOnlyMarkerComponent with image:", selectedImage ? "provided" : "missing");
              ...
              {contextAnswers?.location && (
                <div className="absolute top-3 right-3 z-10">
                  <div className="px-3 py-1.5 bg-[#222]/80 backdrop-blur-sm rounded-full 
                    ...
                    <span>{contextAnswers.location.charAt(0).toUpperCase() + contextAnswers.location.slice(1)}</span>
                  </div>
                </div>
              )}
    ...
    const BasicImageComponent = useCallback(() => {
      console.log("Rendering BasicImageComponent with image:", selectedImage ? "provided" : "missing");
      ...
      return (
        <div className="relative w-full h-full flex items-center justify-center bg-[#232323] p-2">
          <div className="debug-image-container border border-gray-700 p-1 flex items-center justify-center" style={{ width: '90%', height: '90%', background: '#1A1A1A' }}>
            <img
              src={selectedImage}
              alt="Image"
              className="max-w-full max-h-full object-contain"
    ...
    useEffect(() => {
      ...
      console.log("Image URL:", selectedImage?.substring(0, 100) + (selectedImage && selectedImage.length > 100 ? '...' : ''));
      console.log("Image file:", imageFile?.name, imageFile?.size);
      console.log("Points:", points.length);
    }, [step, selectedImage, imageFile, points]);

    useEffect(() => {
      console.log("🔄 firstUploadRef 상태:", firstUploadRef.current ? "있음" : "없음");
    ...
    useEffect(() => {
      ...
      console.log("🔍 firstUploadRef 초기 상태:", firstUploadRef.current ? "있음" : "없음");
        ...
        >
          ...
          >
            {step === 1 ? (
                    ...
                    {showCropper ? (
                      <div className="w-full h-full flex flex-col items-center justify-center bg-[#1A1A1A] overflow-hidden">
                        <div className="w-full h-full relative">
                          {showCropper && <ImageCropper key={`cropper-${selectedImage}`} />}
                        </div>
                      </div>
                    ) : <ImageViewer />}
                  </div>
              ...
              selectedImage ? <ReadOnlyMarkerComponent /> : renderUploadUI()
            )}
          </div>
        </div>
      </div>
