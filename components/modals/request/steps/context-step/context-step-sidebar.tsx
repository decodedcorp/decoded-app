...
export function ContextStepSidebar({
  ...
  const handleSourceClick = (value: string) => {
    ...
    const hasCustomInput = answers.source && answers.source.startsWith(value) && answers.source !== value;
  ...
  const handleDrag = (info: any) => {
    ...
    const newHeight = Math.max(20, Math.min(90, sheetHeight - (info.delta.y * 0.2)));
    ...
    <>
      ...
      <motion.div 
        className={cn(
          ...
          isMobile && "fixed bottom-0 left-0 right-0 rounded-t-2xl shadow-lg z-40"
        )}
        style={isMobile ? {
              ...
              {step === 'location' ? (
                <div className="flex-1 space-y-2.5 overflow-y-auto overflow-x-hidden">
                  {locationOptions.map((option) => (
                    <motion.div
                      ...
                      className={`flex items-center space-x-3 ${isMobile ? 'p-3' : 'p-3.5'} rounded-lg transition-all cursor-pointer overflow-hidden
                ...
                <div className="flex-1 space-y-2.5 overflow-y-auto overflow-x-hidden">
                  {sourceOptions.map((option) => {
                    const hasCustomInput = answers.source && answers.source.startsWith(`${option.value}:`) && answers.source !== option.value;
                    const customInputValue = hasCustomInput ? answers.source?.split(':')?.[1] || '' : '';
                    ...
                    return (
                      ...
                      >
                        {editingSourceId === option.value ? (
                          ...
                          <>
                            <div className={`w-4 h-4 rounded-full flex items-center justify-center
                              ${answers.source === option.value || hasCustomInput
                                ? 'border-[#EAFD66]' 
                                : 'border-zinc-600'} border`}
                            >
                              {(answers.source === option.value || hasCustomInput) && (
                            ...
                            <Label className="flex-1 cursor-pointer text-sm text-zinc-300">
                              ...
                              {hasCustomInput && (
                                <span className={cn(
                                  ...
                                  ✓ {customInputValue.substring(0, isMobile ? 20 : 15)}
                                  {customInputValue.length > (isMobile ? 20 : 15) ? '...' : ''}
                                </span>
                              )}
                            </Label>
                          </>
