...
export function ShareButtons({
  ...
  const findPageImage = () => {
    ...
    for (const selector of ogImageSelectors) {
      ...
      console.log(`${selector} found:`, ogImage ? "Yes" : "No", ogImage?.getAttribute('content'));
    ...
    for (const selector of twitterImageSelectors) {
      ...
      console.log(`${selector} found:`, twitterImage ? "Yes" : "No", twitterImage?.getAttribute('content'));
    ...
    console.log("Large images found:", images.length, images.length > 0 ? images[0].src : "None");
    if (images.length > 0) {
    ...
    console.log("No suitable images found, using default thumbnail:", thumbnailUrl);
  ...
  const openKakaoShareLink = () => {
    ...
    if (isMobile) {
      ...
      window.location.href = `intent://send?text=${encodeURIComponent(currentTitle + "\n" + currentUrl)}#Intent;scheme=kakaolink;package=com.kakao.talk;end`
      ...
      navigator.clipboard.writeText(currentUrl)
        .then(() => {
          if (confirm("URL이 클립보드에 복사되었습니다. 카카오톡을 실행하시겠습니까?")) {
            window.open('https://talk.kakao.com', '_blank')
          }
        })
        .catch(() => {
          // 클립보드 복사 실패 시 대체 방법
          window.open(`https://sharer.kakao.com/talk/friends/picker/link?app_key=${kakaoAppKey}&meta={"title":"${encodeURIComponent(currentTitle)}","description":"${encodeURIComponent(description)}", "image_url":"${encodeURIComponent(getImageUrl())}","link":{"web_url":"${encodeURIComponent(currentUrl)}","mobile_web_url":"${encodeURIComponent(currentUrl)}"}}`, '_blank')
        })
    }
  }
  ...
  const shareTelegram = () => {
    ...
    const telegramShareUrl = `https://t.me/share/url?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(currentTitle)}&image=${imageUrl}`;
  ...
  const shareTwitter = () => {
    ...
    const twitterShareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(currentTitle)}&url=${encodeURIComponent(currentUrl + (currentUrl.includes('?') ? '&' : '?') + '_t=' + Date.now())}`;
    window.open(twitterShareUrl, "_blank");
  }

  // 메시지(SMS) 공유
  const shareSMS = () => {
    const smsShareUrl = `sms:?&body=${encodeURIComponent(currentTitle + ": " + currentUrl)}`
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-md bg-black/90 dark:bg-black border-0 shadow-lg rounded-lg overflow-hidden p-0">
        <DialogHeader className="bg-black/80 dark:bg-black/90 px-4 h-14 border-b border-gray-700 dark:border-gray-800 relative flex items-center justify-center">
          <DialogTitle className="text-base font-medium text-white m-0">{title}</DialogTitle>
          <DialogClose className="absolute right-4 text-white hover:text-white/80 focus:outline-none transition-colors flex items-center justify-center w-8 h-8">
            <X className="h-4 w-4" />
        ...
        <div className="grid grid-cols-3 gap-3 p-4">
          {canShare && (
            ...
            >
              <div className="w-8 h-8 flex items-center justify-center bg-blue-500 text-white rounded-full mb-1">
              ...
              <span className="text-xs font-medium text-white">기본 공유</span>
            </Button>
          )}
          <Button 
            variant="outline" 
          ...
          >
            <div className="w-8 h-8 flex items-center justify-center bg-[#FEE500] text-black rounded-full mb-1">
              <svg width="16" height="16" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
                <path d="M128 36C70.562 36 24 72.713 24 118c0 29.279 19.466 54.97 48.748 69.477-1.593 5.494-10.237 35.344-10.581 37.689 0 0-.207 1.762.934 2.434s2.483.15 2.483.15c3.272-.457 37.943-24.811 43.806-29.052 5.9.849 11.93 1.302 18.11 1.302 57.438 0 104-36.712 104-82c0-45.287-46.562-82-104-82z" fill="#3A1D1D"/>
              </svg>
            </div>
            <span className="text-xs font-medium text-white">카카오톡</span>
          </Button>
          ...
          <Button 
            ...
            className="flex flex-col items-center justify-center h-16 bg-black/50 hover:bg-black/70 border border-gray-700 hover:border-primary/50 rounded-lg transition-colors p-1"
          >
            <div className="w-8 h-8 flex items-center justify-center bg-[#0088cc] text-white rounded-full mb-1">
            ...
            <span className="text-xs font-medium text-white">텔레그램</span>
          </Button>
          ...
          <Button 
            ...
            className="flex flex-col items-center justify-center h-16 bg-black/50 hover:bg-black/70 border border-gray-700 hover:border-primary/50 rounded-lg transition-colors p-1"
          >
            <div className="w-8 h-8 flex items-center justify-center bg-black text-white border border-gray-600 rounded-full mb-1">
              <svg width="16" height="16" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="white">
