...
export const LoginButton = React.memo(function LoginButton() {
  ...
  useEffect(() => {
    ...
    if (localLoginState === null) {
      ...
      const initialState = !!(userDocId && accessToken);
    ...
    const detectLoginState = () => {
      ...
      const currentLoginState = !!(userDocId && accessToken);
  ...
  const handleLoginSuccess = useCallback(() => {
    ...
    const isNowLoggedIn = !!(userDocId && accessToken);
      ...
      if (timeDiff < 10000) {
        ...
        const processToken = async () => {
          try {
            ...
            console.error(
              "[LoginButton] Mobile login processing failed:",
              error
            );
            localStorage.removeItem("TEMP_ID_TOKEN");
            localStorage.removeItem("LOGIN_TIMESTAMP");
  ...
  // 로그인 상태에 따른 텍스트 표시
  const buttonText = (() => {
    if (!isInitialized) return t.header.nav.login.button.login;
    if (isLoading) return t.header.nav.login.button.loading;

    // 로컬 로그인 상태를 우선적으로 사용 (더 빠른 UI 업데이트)
    if (localLoginState !== null) {
      return localLoginState
        ? t.header.nav.login.button.myPage
        : t.header.nav.login.button.login;
    }
  ...
  const handleOpenModal = useCallback(
    (e?: React.MouseEvent) => {
      ...
      const isLoggedIn = localLoginState ?? isLogin;

      if (isLoggedIn) {
        router.push(`/profile/${window.sessionStorage.getItem("USER_DOC_ID")}`);
      } else {
        if (!isLoginModalOpen) {
          openLoginModal();
        }
    ...
    <>
      <button
        ...
        className={cn(
          ...
          {
            "text-[#EAFD66] hover:text-[#EAFD66]/90 active:text-[#EAFD66]/80":
              localLoginState ?? isLogin,
          }
        )}
        data-modal-trigger="true"
        data-no-close-on-click="true"
        data-login-state={
          localLoginState ?? isLogin ? "logged-in" : "logged-out"
        }
      >
