...
export function StatusModal({
  ...
  const { title, message } =
    messageKey && type !== "loading"
      ? {
          title:
            (
              t.common.status.messages[
                type as keyof typeof t.common.status.messages
              ] as any
            )?.[messageKey]?.title ?? t.common.status[type],
          message:
            (
              t.common.status.messages[
                type as keyof typeof t.common.status.messages
      ...
      >
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            className={cn(buttonStyles.base, buttonStyles[type])}
          >
