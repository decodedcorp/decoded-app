...
import { Card } from "../../../../components/ui/cardui/card";
        ...
        <div className="relative">
          ...
          {/* <div className="absolute top-2 right-2">
            ...
            >
              <Bookmark
                className={`h-4 w-4 ${
                  isBookmarked ? "text-[#EAFD66] fill-[#EAFD66]" : "text-white"
                }`}
              />
            </button>
          </div> */}
...
const BoardCard = ({
  ...
  return (
    <motion.div
      ...
      className={
        isActive ? "" : "opacity-40 hover:opacity-70 transition-opacity"
      }
      onClick={onClick}
    >
...
const FilterButton = ({
  ...
  const buttonStyle = isActive
    ? "text-black shadow-md transform-gpu scale-105 transition-all duration-200"
    : "bg-transparent text-gray-400 hover:text-black transition-all duration-200";

  return (
    <Button
      size="sm"
      className={`rounded-lg text-xs flex items-center text-gray-400 ${buttonStyle} ${pretendardBold.className}`}
      onClick={onClick}
    >
      {icon}
      {label}
  ...
  return (
    <div className="flex items-center justify-between border-b border-white/10 pb-4">
      <div className="flex space-x-3">
        <button
          className={`text-sm font-medium relative pb-4 ${
            viewMode === "pins"
              ? "text-white"
...
function useTabInfiniteScroll(type: TabType, userDocId: string) {
  ...
  const fetchRequestData = async ({
    ...
    if (!userDocId) throw new Error("사용자 정보를 찾을 수 없습니다");

    try {
      console.log("[요청] API 호출 시작:", { userDocId, next_id, limit });
      const response = await mypageService.getUserRequests(userDocId, {
        next_id,
        limit,
      });
  ...
  const fetchProvideData = async ({
    ...
    try {
      console.log("[제공] API 호출 시작:", { userDocId, next_id, limit });
      const response = await mypageService.getUserProvides(userDocId, {
        next_id,
        limit,
      });
  ...
  const fetchLikeData = async ({
    ...
    try {
      console.log("[좋아요] API 호출 시작:", { userDocId, next_id, limit });
      const response = await mypageService.getUserLikedImages(userDocId, {
        next_id,
        limit,
      });
  ...
  const wrappedFetchFunction = useMemo(() => {
    return ({ next_id, limit }: { next_id?: string; limit?: number }) => {
      return fetchFunction({ userDocId, next_id, limit });
    };
  }, [fetchFunction, userDocId]);

  // useInfiniteScroll 훅으로 데이터 로딩
  const { items, isLoading, error, hasMore, loadingRef } = useInfiniteScroll({
  ...
  if (items.length !== countRef.current) {
    ...
    console.log(
      `[TabInfiniteScroll][${type}] 카운트 업데이트:`,
      countRef.current
    );
  }
...
export function ActivityTabs({ userDocId }: ActivityTabsProps) {
  const { t } = useLocaleContext();
  const [viewMode, setViewMode] = useState<"pins" | "boards">("pins");
  const [activeFilter, setActiveFilter] = useState<TabType>("request");

  // 각 탭의 무한 스크롤 훅 사용
  const requestData = useTabInfiniteScroll("request", userDocId);
  const provideData = useTabInfiniteScroll("provide", userDocId);
  const likeData = useTabInfiniteScroll("like", userDocId);
  ...
  if (
    ...
    provideData.isLoading &&
    provideData.items.length === 0 &&
    likeData.isLoading &&
    likeData.items.length === 0
  ) {
    return (
  ...
  if (
    ...
    return (
      <div className="text-center py-8 bg-[#1A1A1A] border border-white/5 rounded-lg">
        <p className="text-white/60 mb-4">활동 내역이 없습니다.</p>
  ...
  return (
    <div className="container mx-auto space-y-6 mt-20">
      {/* 핀/보드 탭 헤더 */}
