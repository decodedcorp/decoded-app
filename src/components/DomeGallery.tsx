...
const getDataNumber = (el: Element, name: string, fallback: number) => {
  const attr = (el as HTMLElement).dataset[name] ?? el.getAttribute(`data-${name}`);
    ...
    useEffect(() => {
      ...
      `;
        ...
        if (originalImg) {
          ...
          (img as HTMLImageElement).style.cssText = 'width: 100%; height: 100%; object-fit: cover;';
