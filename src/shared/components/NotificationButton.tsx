...
export function NotificationButton({ unreadCount }: NotificationButtonProps) {
  ...
  const currentUserId = currentUser?.doc_id || '';
  ...
  const receivedInvitations = allInvitations.filter((inv) => {
    const inviteeId = inv.invitee?.id || '';
  ...
  const handleMarkAllRead = () => {
    markAllReadMutation.mutate({} as any);
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {actualUnreadCount > 0 && (
          <span
            className={`absolute -top-1 -right-1 text-white text-xs rounded-full h-5 min-w-[20px] px-1 flex items-center justify-center font-medium ${badgeColor}`}
          >
            ...
            {notificationsLoading || invitationsLoading ? (
              ...
              <>
                ...
                {(receivedInvitations.length > 0 || sentInvitations.length > 0) &&
                  notifications.length > 0 && <div className="border-b border-zinc-800" />}
