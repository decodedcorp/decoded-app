...
export function SidebarToggleItem({
  ...
  const buttonContent = (
    <>
      ...
      {isExpanded ? (
        <ChevronDownIcon className="w-4 h-4 lg:w-4 lg:h-4 flex-shrink-0" />
      ) : (
        <ChevronRightIcon className="w-4 h-4 lg:w-4 lg:h-4 flex-shrink-0" />
      )}
    </>
  );
  ...
  const buttonClasses = `
    ...
    text-sm lg:text-sm font-normal transition-all duration-200 w-full text-left cursor-pointer
