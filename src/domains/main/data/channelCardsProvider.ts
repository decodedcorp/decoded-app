...
export const DEFAULT_CHANNEL_ID = '688a317213dbcfcd941c85b4';
...
const transformContentToCard = (content: APIContent): Card => {
  ...
  const title = content.link_preview_metadata?.title || 
                content.ai_gen_metadata?.summary?.slice(0, 60) || 
                `Content ${content.id.slice(-8)}`;
                
  const authorName = content.link_preview_metadata?.site_name || 
                     (content.category ? `${content.category.charAt(0).toUpperCase()}${content.category.slice(1)}` : 
  ...
  return {
    ...
    metadata: {
      ...
      keywords: content.ai_gen_metadata?.qa_list?.map(qa => qa.question.split(' ').slice(0, 3).join(' ')) || [],
...
const transformAPIResponse = (apiResponse: ContentListResponse): CardsResponse => {
  const cards = apiResponse.contents?.map((content: any) => transformContentToCard(content as APIContent)) || [];
