  ...
  (
    ...
    useEffect(() => {
      if (isCentering && centerAnimationProgress > 0) {
        const newX =
          centerAnimationStart.x +
          (centerAnimationTarget.x - centerAnimationStart.x) * centerAnimationProgress;
        const newY =
          centerAnimationStart.y +
          (centerAnimationTarget.y - centerAnimationStart.y) * centerAnimationProgress;
    ...
    useEffect(() => {
      if (isZooming && zoomAnimationProgress > 0) {
        const currentZoom =
          zoomAnimationStart + (targetZoomLevel - zoomAnimationStart) * zoomAnimationProgress;
    ...
    useEffect(() => {
      if (
        ...
        const availableWidth = isSidebarOpen
          ? containerDimensions.width * 0.65
          : containerDimensions.width; // 사이드바가 열려있으면 65%만 사용
        ...
        const targetX =
          -(selectedImagePosition.x * gridWidth) + (availableWidth - gridWidth) / 2 - sidebarOffset;
        const targetY =
          -(selectedImagePosition.y * gridHeight) + (containerDimensions.height - gridHeight) / 2;
      ...
      if (isDragging) {
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            const maxDistance =
              Math.max(containerDimensions.width, containerDimensions.height) +
              viewportMargin * 0.3; // 더 작은 거리로 최적화
