...
export const SearchAutocomplete = React.memo(({
  ...
  const handleItemClick = useCallback((item: AutocompleteItem, index: number) => {
    onSelect(item);
    onClose?.();
  ...
  if (isLoading && !hasResults) {
    return (
      <div className={clsx(
        ...
        'border border-zinc-600/50 rounded-lg shadow-xl z-50',
        className
      )}>
  ...
  return (
    <div
      ...
      className={clsx(
        'absolute top-full left-0 right-0 mt-2 bg-zinc-800/95 backdrop-blur-sm',
        'border border-zinc-600/50 rounded-lg shadow-xl z-50 max-h-96 overflow-y-auto',
        className
      )}
      role="listbox"
    ...
    >
      ...
      {groupedResults.contents.length > 0 && (
        <div role="group" aria-labelledby="contents-heading">
          ...
          {groupedResults.contents.slice(0, maxResults - groupedResults.channels.length).map((item) => {
            const resultIndex = results.indexOf(item);
            return (
              <div
                key={`content-${item.id}`}
