...
export const MasonryItemsGrid = () => {
  ...
  const loadInitialItems = () => {
    ...
    setCurrentPage(1); // 첫 페이지 로드 완료
  };

  // 추가 데이터 로드
  const loadMoreItems = () => {
    const nextGroupKey = currentPage + 1;
    
    // 데이터가 부족하다면 기존 데이터 재활용
    const newItems = items.length > 0 
      ? items.slice(0, Math.min(PAGE_SIZE, items.length)).map(item => ({
          ...item,
          key: item.key + (nextGroupKey * PAGE_SIZE),
          groupKey: nextGroupKey
        }))
      : fetchDummyItems(nextGroupKey, PAGE_SIZE);
    
    setItems(prevItems => [...prevItems, ...newItems]);
    setCurrentPage(nextGroupKey);
  };
  ...
  return (
    <div ref={containerRef} className="w-full">
      <MasonryInfiniteGrid
        ...
        placeholder={
          <div className="animate-pulse bg-gray-200 w-full h-full rounded-md" style={{
            aspectRatio: '1/1',
          }}></div>
        }
        onRequestAppend={(e) => {
          ...
          e.wait();
          e.currentTarget.appendPlaceholders(PAGE_SIZE / 2, nextGroupKey);
          
          console.log(`Adding placeholders for group ${nextGroupKey}`);
          ...
          setTimeout(() => {
            ...
            console.log(`Adding ${newItems.length} new items for group ${nextGroupKey}`);
