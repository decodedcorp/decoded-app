...
export function FloatingBox({
  ...
  const getDetailUrl = (image: ImageInfo, isLarge: boolean) => {
    ...
    // 항상 imageId는 포함
    params.append('imageId', image.doc_id);
    
    // 작은 박스이고 아이템 정보가 있는 경우에만 itemId와 showList 추가
    if (!isLarge && image.item?.item?._id) {
      params.append('itemId', image.item.item._id);
      params.append('showList', 'true');
    }
  ...
  const handleClick = (e: React.MouseEvent) => {
    if (image) {
      const url = getDetailUrl(image, isLarge);
      // URL은 Link 컴포넌트에서 사용되므로 여기서는 별도 처리가 필요 없음
    }
  };
    ...
    >
      {image ? (
        <Link
          ...
          className="block w-full h-full relative z-[35]"
          onClick={handleClick}
          prefetch={false}
        >
          {image.img_url ? (
