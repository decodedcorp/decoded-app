...
const generatePositions = (count: number) => {
  ...
  return (resource: ItemDoc | ImageDoc, index: number) => {
    ...
    const areaIndex = availableAreas[Math.floor(Math.random() * availableAreas.length)];
...
export function BoxContainer({ sizeMode, onBoxHover, resources }: BoxContainerProps) {
  ...
  useEffect(() => {
    if (!initialized.current && resources.length > 0) {
      ...
      if (positionGenerator) {
        const initialBoxData = resources.map((resource, index) => positionGenerator(resource, index));
  ...
  const handleResize = useCallback(() => {
    ...
    resizeTimeoutRef.current = setTimeout(() => {
      if (resources.length > 0) {
        ...
        if (positionGenerator) {
          setBoxData(prevData => 
            prevData.map((data, index) => positionGenerator(data.resource, index))
          );
        }
      }
    }, 200);
