...
import type { FetchFilteredImagesParams, ImagePageWithPostId, ImageWithPostId } from "./images";
import { fetchImagesByPostImage } from "./images";
...
export async function fetchUnifiedImages(
  ...
  if (postBasedResult.items.length < (queryParams.limit || 50)) {
    const remainingSlots = (queryParams.limit || 50) - postBasedResult.items.length;
  ...
  if (process.env.NODE_ENV === "development" || process.env.ENABLE_QUERY_STATS) {
    const fromPost = postBasedResult.stats?.fromPostImage || postBasedResult.items.length;
    const fromOrphans = orphanResult.stats?.fromOrphans || orphanResult.items.length;
    ...
    console.log("[fetchUnifiedImages] Stats:", {
      ...
      orphanRate: fromOrphans > 0 ? ((fromOrphans / (fromPost + fromOrphans)) * 100).toFixed(2) + "%" : "0%",
    });
  }
  ...
  return {
    ...
    hasMore,
    stats: {
      fromPostImage: postBasedResult.stats?.fromPostImage || postBasedResult.items.length,
