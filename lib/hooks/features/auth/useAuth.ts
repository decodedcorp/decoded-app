...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    setIsLogin(!!(userDocId && accessToken));
    setIsInitialized(true);
  }, []);
  ...
  useEffect(() => {
    ...
    const login = async (token: string) => {
      ...
      try {
        ...
        // 백엔드 응답 데이터 저장
        const { salt, doc_id, access_token, has_sui_address } = loginRes.data;
        ...
        if (!has_sui_address) {
          ...
          const res = await networkManager.request(
            `/user/${doc_id}/address/${sui_acc}`,
            "PATCH",
            null
          );
          if (res.status_code !== 200) {
            console.error("[Auth] Sui address patch failed:", res);
          }
        }
      ...
      if (providedToken) {
        ...
        try {
          ...
          if (!popup) {
            // 팝업이 차단된 경우
            logError(
              "[Auth] 팝업을 열 수 없습니다. 브라우저 설정을 확인하세요."
            );
            setIsLoading(false);
            return;
          }
          ...
          const messageListener = async (event: MessageEvent) => {
            ...
            try {
              ...
              if (!sub || !iss || !aud) {
                throw new Error(
                  "[Auth] Missing required fields in decoded token"
                );
              }
              ...
              const { salt, doc_id, access_token, has_sui_address } =
                loginRes.data;
              logDebug("[Auth] Saving user session data...");
              window.sessionStorage.setItem("ACCESS_TOKEN", access_token);
              window.sessionStorage.setItem("USER_DOC_ID", doc_id);
  ...
  const handleDisconnect = () => {
    ...
    const sessionInfo = {
      ...
      SUI_ACCOUNT:
        window.sessionStorage.getItem("SUI_ACCOUNT")?.substring(0, 10) + "...",
  ...
  const checkLoginStatus = () => {
    ...
    if (shouldCheck) {
      ...
      setIsLogin(!!(userDocId && accessToken && isTokenValid));
    }
  };
  ...
  const checkTokenStatus = () => {
    ...
    logDebug("Token status check:", {
      ...
      hasUserDocId: !!userDocId,
      isLoginState: !!(accessToken && userDocId),
    });
