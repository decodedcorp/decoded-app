...
export function useScrollAnimation(
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (Array.isArray(threshold)) {
      threshold.forEach((t) => {
        if (t < 0 || t > 1) {
          console.error(`[useScrollAnimation] Invalid threshold value: ${t}. Must be between 0.0 and 1.0`)
        }
      })
    } else if (threshold < 0 || threshold > 1) {
      console.error(`[useScrollAnimation] Invalid threshold: ${threshold}. Must be between 0.0 and 1.0`)
    }

    // Validate rootMargin (basic CSS string check)
    if (!/^(-?\d+(%|px)\s*){1,4}$/.test(rootMargin.trim())) {
      console.error(`[useScrollAnimation] Invalid rootMargin: "${rootMargin}". Must be a valid CSS margin string (e.g., "0px 0px -10% 0px")`)
    }
  }
  ...
  const elementStateMap = useRef<WeakMap<Element, { imageLoaded: boolean }>>(new WeakMap())
      ...
      (entries) => {
        ...
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            ...
            const state = elementStateMap.current.get(element) || { imageLoaded: false }
  ...
  }, [threshold, rootMargin, root, onEnter, onExit, enableLazyLoad, enableAnimation])
