...
test.describe('Scroll Animation System', () => {
  ...
  test('T007: cards should have is-visible class added when scrolled into view', async ({ page }) => {
  ...
  test('T008: multiple cards should animate with staggered delays (cascading effect)', async ({ page }) => {
  ...
  test('T009: cards should have is-hidden class when scrolled away', async ({ page }) => {
    const firstCard = page.locator('.js-observe').first()
...
test.describe('Lazy Image Loading (User Story 2)', () => {
  ...
  test('T018: images should load only when scrolled near (lazy loading)', async ({ page }) => {
  ...
  test('T019: images should not reload when re-entering viewport (data-loaded="true")', async ({ page }) => {
    const lazyImage = page.locator('img[data-src]').first()
  ...
  test('T020: above-the-fold images should load immediately', async ({ page }) => {
...
test.describe('Performance Optimization (User Story 3)', () => {
  ...
  test('T028: animations should maintain 60fps (frame time <16.67ms)', async ({ page }) => {
    const cards = page.locator('.js-observe')
  ...
  test('T029: Core Web Vitals should meet targets (CLS ≤ 0.1, LCP ≤ 2.5s)', async ({ page }) => {
  ...
  test('T030: animations should work smoothly on mobile devices', async ({ page, context }) => {
    // Emulate iPhone 12
