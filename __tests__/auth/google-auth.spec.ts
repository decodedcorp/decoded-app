...
test.describe('Google Authentication Flow', () => {
  ...
  test('should display Google login button', async ({ page }) => {
    ...
    const loginButton = page.locator('button:has-text("Continue with Google"), button:has-text("Login"), button:has-text("Sign in")').first();
  ...
  test('should handle OAuth callback with error', async ({ page }) => {
    ...
    await expect(page.locator('text*="error", text*="Error", text*="OAuth"')).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Auth Components Integration', () => {
  test('should render login form correctly', async ({ page }) => {
    ...
    const hasLoginUI = await page.locator('button, [class*="login"], [class*="auth"], text*="Login", text*="Sign"').count() > 0;
    console.log('Login UI check:', { hasLoginUI });
